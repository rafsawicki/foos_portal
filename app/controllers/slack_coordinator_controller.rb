class SlackCoordinatorController < ApplicationController
  ...
  def receive
    ...
    when 'foos'
      begin
        ...
        if game.in_progress?
          ...
          json_result[:text] = "A game has already been started. Need #{game.players_needed_to_start} more #{pluralize_players(game.players_needed_to_start)} to start. Type _.in_ or _+_ to join."
        end
      end
    ...
    when 'quit'
      ...
      if success
        json_result[:text] = 'Game has been abandoned. Type _.foos_ or _+_ to start a new one.'
      else
        json_result[:text] = 'There is no game to abandon. Type _.foos_ or _+_ to start a new one.'
      end
    ...
    when 'out', '-'
      begin
        ...
        if game
          ...
          json_result[:text] = 'All players have quit the game. Type _.foos_ or _+_ to start a new one.'
        end
        ...
        json_result[:text] = 'There is no game to quit. Type _.foos_ or _+_ to start a new one.'
